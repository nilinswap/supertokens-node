...
exports.handleNonErrorInstance = exports.attachCreateOrRefreshSessionResponseToExpressRes = exports.normaliseSameSiteOrThrowError = exports.validateAndNormaliseUserInput = exports.getURLProtocol = exports.getTopLevelDomainForSameSiteResolution = exports.normaliseSessionScopeOrThrowError = exports.sendTokenTheftDetectedResponse = exports.sendUnauthorisedResponse = exports.sendTryRefreshTokenResponse = void 0;
...
function sendTryRefreshTokenResponse(recipeInstance, _, __, response) {
    return __awaiter(this, void 0, void 0, function* () {
        (0, utils_2.sendNon200Response)(response, "try refresh token", recipeInstance.config.sessionExpiredStatusCode);
    });
}
...
function sendUnauthorisedResponse(recipeInstance, _, __, response) {
    return __awaiter(this, void 0, void 0, function* () {
        (0, utils_2.sendNon200Response)(response, "unauthorised", recipeInstance.config.sessionExpiredStatusCode);
    });
}
...
function getTopLevelDomainForSameSiteResolution(url) {
    ...
    if (
        ...
        (0, utils_1.isAnIpAddress)(hostname)
    ) {
...
function validateAndNormaliseUserInput(recipeInstance, appInfo, config) {
    ...
    if (
        ...
        !cookieSecure &&
        !(
            (topLevelAPIDomain === "localhost" || (0, utils_1.isAnIpAddress)(topLevelAPIDomain)) &&
            (topLevelWebsiteDomain === "localhost" || (0, utils_1.isAnIpAddress)(topLevelWebsiteDomain))
        )
    ) {
...
function attachCreateOrRefreshSessionResponseToExpressRes(config, res, response) {
    ...
    (0, cookieAndHeaders_1.setFrontTokenInHeaders)(
        res,
        response.session.userId,
        response.accessToken.expiry,
        response.session.userDataInJWT
    );
    (0, cookieAndHeaders_1.attachAccessTokenToCookie)(config, res, accessToken.token, accessToken.expiry);
    (0, cookieAndHeaders_1.attachRefreshTokenToCookie)(config, res, refreshToken.token, refreshToken.expiry);
    (0, cookieAndHeaders_1.setIdRefreshTokenInHeaderAndCookie)(
        config,
        res,
    ...
    if (response.antiCsrfToken !== undefined) {
        (0, cookieAndHeaders_1.setAntiCsrfTokenInHeaders)(res, response.antiCsrfToken);
    }
}
exports.attachCreateOrRefreshSessionResponseToExpressRes = attachCreateOrRefreshSessionResponseToExpressRes;
function handleNonErrorInstance(err, handler) {
    if (err instanceof Error) {
        return handler(err);
    }
    throw new Error("nothing exceptional");
}
exports.handleNonErrorInstance = handleNonErrorInstance;
